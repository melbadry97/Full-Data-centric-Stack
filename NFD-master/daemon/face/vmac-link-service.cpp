...
ssize_t
VMACLinkService::getEffectiveMtu() const
{
...
bool
VMACLinkService::canOverrideMtuTo(ssize_t mtu) const
{
...
void
VMACLinkService::sendLpPacket(lp::Packet&& pkt, const EndpointId& endpointId, int type, uint16_t seq)
{
  const ssize_t mtu = getEffectiveMtu();
  ...
  this->sendPacket(block, endpointId, type, seq);
}
...
VMACLinkService::doSendInterest(const Interest& interest, const EndpointId& endpointId)
{
  ...
  this->sendNetPacket(0, std::move(lpPacket), endpointId); // v-mac send point -> Interest
}
...
VMACLinkService::doSendData(const Data& data, const EndpointId& endpointId)
{
  ...
  this->sendNetPacket(1, std::move(lpPacket), endpointId); // v-mac send point -> Data
}
...
VMACLinkService::doSendNack(const lp::Nack& nack, const EndpointId& endpointId)
{
  ...
  this->sendNetPacket(3, std::move(lpPacket), endpointId); // v-mac send point -> NACK
}
...
void
VMACLinkService::sendNetPacket(int type, lp::Packet&& pkt, const EndpointId& endpointId)
{
  ...
  if (m_options.reliabilityOptions.isEnabled && frags.front().has<lp::FragmentField>()) {
    m_reliability.handleOutgoing(frags, std::move(pkt), (type == 0));
  }

  for (lp::Packet& frag : frags) {
    if(frag.has<lp::SequenceField>())
    {
      seq = frag.get<lp::SequenceField>();
    }
    this->sendLpPacket(type, seq, std::move(frag), endpointId);
  }
}
