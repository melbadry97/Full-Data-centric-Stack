...
protected: // lower interface to be invoked in subclass (send path termination)
  /** \brief send a lower-layer packet via V-MAC Transport to \p endpoint
   */
  void
  sendPacket(int type, uint16_t seq, const Block& packet, const EndpointId& endpoint);
...
inline void
LinkService::sendPacket(int type, uint16_t seq, const Block& packet, const EndpointId& endpoint)
{
  m_transport->send(type, seq, packet, endpoint);
}
